...
class MetricsCollector:
    ...
    def __init__(self, data_dir: str = "./data"):
        ...
        self.user_stats: dict[int, dict[str, Any]] = defaultdict(
            lambda: {
                "commands_used": 0,
                "total_time": 0.0,
                "last_activity": None,
    ...
    def record_command_execution(
        self, command: str, user_id: int, execution_time: float, success: bool = True, error_msg: str | None = None
    ) -> None:
    ...
    @log_call("metrics")
    def record_error(
        self, error_type: str, user_id: int, error_msg: str, context: dict[str, Any] | None = None
    ) -> None:
        """Записывает ошибку."""
    ...
    def get_user_statistics(self) -> dict[str, Any]:
        ...
        for user_id, data in self.user_stats.items():
            favorite_command = (
                max(data["favorite_commands"].items(), key=lambda x: x[1])[0] if data["favorite_commands"] else None
            )

            stats[str(user_id)] = {
                "commands_used": data["commands_used"],
                "avg_time_per_command": round(data["total_time"] / data["commands_used"], 3)
                if data["commands_used"] > 0
                else 0,
    ...
    def get_usage_patterns(self) -> dict[str, Any]:
        ...
        return {
            ...
            "most_used_commands": sorted(
                [(cmd, data["count"]) for cmd, data in self.command_stats.items()], key=lambda x: x[1], reverse=True
            )[:10],
        }
    ...
    def get_audit_trails(self, limit: int = 100) -> list[dict[str, Any]]:
        ...
        return self.audit_trails[-limit:] if limit > 0 else self.audit_trails
    ...
    async def save_metrics(self) -> None:
        ...
        try:
            metrics_data = {
                ...
                "command_stats": {
                    cmd: {**data, "users": list(data["users"])} for cmd, data in self.command_stats.items()
                },
                "user_stats": {
                    str(uid): {**data, "favorite_commands": dict(data["favorite_commands"])}
    ...
    async def load_metrics(self) -> None:
        ...
        try:
            ...
            self.usage_patterns = metrics_data.get(
                "usage_patterns",
                {
                    "hourly_usage": defaultdict(int),
                    "daily_usage": defaultdict(int),
