...
async def handle_on(message: Message) -> None:
    ...
    if input_arg in all_paths:
        ...
        if not Path(input_arg).is_absolute():
            system_dirs = [
            ...
            for sys_dir in system_dirs:
                for ext in extensions:
                    if ext and input_arg.lower().endswith(ext):
                        candidate = str(Path(sys_dir) / input_arg)
                    else:
                        candidate = str(Path(sys_dir) / (input_arg + ext))

                    if Path(candidate).exists():
                        file_path = candidate
                        custom_path = True
                        process_key = str(Path(file_path).resolve())
                        new_path_found = True
                        found = True
                if found:
                    break
        ...
        if not found:
            abs_candidate = str(Path(input_arg).resolve())
            if Path(abs_candidate).exists():
    ...
    reply_msg_base = f"✅ Успешно запущен: {Path(file_path).name}"
    if custom_path:
        reply_msg_base += f"\n📁 Путь: {file_path}"
    if arguments:
    ...
    if new_path_found and input_arg not in all_paths:
        ...
        alias_name = Path(input_arg).name
    ...
    try:
        if admin_mode:
            ...
            creationflags = subprocess.CREATE_NO_WINDOW if hidden_mode else subprocess.CREATE_NEW_CONSOLE
...
async def handle_save_path_callback(call: CallbackQuery) -> None:
    try:
        ...
        if config.add_user_path(call.from_user.id, name, path):
            await call.message.edit_text(f"✅ Путь сохранен!\n\nИмя: <code>{name}</code>\nПуть: {path}")
        else:
...
async def handle_off(message: Message) -> None:
    ...
    if not matched_proc:
        ...
        for name, proc in active_processes.items():
            if Path(name).name.lower().startswith(target_lower):
                matched_proc = proc
    ...
    if matched_proc.poll() is not None:
        ...
        await message.answer(f"ℹ️ Процесс '{Path(matched_name).name}' уже завершен")
        return
    ...
    await manager.create_confirmation(
        ...
        warning_message=action_config["warning"].format(
            action_data=f"Остановка процесса: {Path(matched_name).name} (PID: {matched_proc.pid})"
        ),
        timeout=action_config["timeout"],
    )
