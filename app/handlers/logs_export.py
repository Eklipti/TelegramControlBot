...
async def handle_logs_export_json(message: Message) -> None:
    ...
    try:
        ...
        file_input = FSInputFile(
            export_path, filename=f"TelegramControlBot_logs_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
...
async def handle_logs_export_csv(message: Message) -> None:
    ...
    try:
        ...
        file_input = FSInputFile(
            export_path, filename=f"TelegramControlBot_logs_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        )
...
async def handle_logs_export_xml(message: Message) -> None:
    ...
    try:
        ...
        file_input = FSInputFile(
            export_path, filename=f"TelegramControlBot_logs_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xml"
        )
...
async def handle_logs_export_txt(message: Message) -> None:
    ...
    try:
        ...
        file_input = FSInputFile(
            export_path, filename=f"TelegramControlBot_logs_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        )
...
async def handle_logs_filter(message: Message) -> None:
    ...
    try:
        ...
        filtered_logs = centralized_logger.get_logs(
            level=level, logger_name=logger_name, start_time=start_time, limit=limit
        )
        ...
        trace_function_exit(
            "handle_logs_filter", result=f"{len(filtered_logs)} logs", logger_name="logs_export_handler"
        )
