# ControlBot v2.2.1 - Telegram-бот для удалённого управления ПК

Мощный Telegram-бот для удалённого управления компьютером через Telegram с использованием Aiogram 3. ControlBot предоставляет комплексные возможности удалённого управления для Windows-систем с акцентом на безопасность, мониторинг и удобство использования.

## 🆕 Что нового в v2.2.1

- **📊 Централизованная система логирования** с экспортом в JSON, CSV, XML, TXT
- **📈 Система метрик и мониторинга** с детальной статистикой использования
- **🔒 Улучшенная система безопасности** с подтверждениями для опасных действий
- **🖥️ Headless-режим** с ленивыми импортами GUI модулей
- **📋 Новые команды статистики** и экспорта логов
...
5. **Настройте бота**
   ...
   - Настройте разрешённые ID пользователей
...
### Console script (после установки)
```bash
# Установка пакета
pip install -e .
...
### Windows batch-файл
```bash
# Использование предоставленного Windows batch-файла
scripts/windows/ControlBot.bat
```
...
- **[Планы на будущее](./FUTURE.md)** - предстоящие функции и улучшения
- **[Руководство по тестированию](./TESTING.md)** - информация о тестировании и разработке
- **[Headless-режим](./HEADLESS.md)** - запуск в headless-окружениях
- **[Руководство разработчика](./DEVELOPMENT.md)** - документация для разработчиков
- **[Политика безопасности](./SECURITY.md)** - руководство по безопасности и сообщения об уязвимостях
- **[История изменений](./CHANGELOG.md)** - история версий и изменений
- **[Руководство по миграции v2.2](./MIGRATION_v2.2.md)** - руководство по миграции на v2.2.0
...
- **Контекстная справка**: Подробная справка для каждой команды
- **Inline-клавиатуры**: Кнопки быстрого доступа для частых операций
...
- **Интуитивный интерфейс**: `/start` для приветствия, `/help` для списка команд
...
- `@pytest.mark.headless` - тесты, которые работают в headless-окружении
...
- `run_tests.py` - универсальный раннер тестов

#### Планируемый запуск тестов

**Все тесты (локальная разработка):**
```bash
...
**Только headless-тесты (CI/GitHub Actions):**
```bash
...
- **Поддержка headless**: Автоматическое определение headless-режима
